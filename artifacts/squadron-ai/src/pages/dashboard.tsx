...
import {
  AlertTriangle, Activity, Zap, ShieldAlert, Target,
  Skull, Lock, Eye, RotateCcw, Radio, Wifi, TrendingUp,
  TrendingDown, AlertCircle, CheckCircle2, XCircle,
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: bot, isLoading: botLoading } = useBotStatus();
  ...
  const allTimePnl = pnl?.allTimePnlUsd ?? 0;
  ...
  const lossFromStart = Math.min(0, circuit?.dailyGainPct ?? 0);
  const drawdownPct = Math.abs(Math.min(0, lossFromStart));
  ...
  return (
    <Layout>
      <div className="max-w-md mx-auto px-3 pb-4 space-y-3">
        ...
        {isPaper && (
          <div className="bg-yellow-400/10 border border-yellow-400/50 rounded p-2 flex items-center gap-2">
            <AlertCircle size={12} className="text-yellow-400 shrink-0" />
            <span className="text-[9px] text-yellow-400 font-bold uppercase tracking-wider">SIMULATION MODE — No real money traded</span>
          </div>
        )}
        {systemAtRisk && (
        ...
        {circuitState === "FORTRESS_LOCKED" && (
          <div className="bg-losses/10 border border-losses/50 rounded p-2">
            <p className="text-[9px] text-losses font-bold uppercase tracking-wider">🔒 FORTRESS LOCKED: DAILY LOSS LIMIT REACHED</p>
            <p className="text-[8px] text-muted-foreground mt-0.5">Scanner disabled for 12 hours. Use reset to manually unlock.</p>
        ...
        {circuitState === "GLOBAL_FLOOR_HIT" && (
          <div className="bg-red-950 border border-red-600 rounded p-2">
            ...
            <Button size="sm" className="mt-1.5 h-5 text-[9px]" variant="destructive" onClick={() => resetFortress.mutate()}>Confirm Manual Restart</Button>
          </div>
        )}
        ...
        <div className="bg-card border border-card-border rounded-lg p-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {bot?.conservativeMode && <Badge variant="outline" className="text-[9px] text-blue-400 border-blue-400/30 uppercase">Conservative</Badge>}
            </div>
            <Button
              size="sm"
              variant="destructive"
              className="h-6 text-[9px] px-2 font-bold uppercase tracking-wider"
              onClick={() => { if (confirm("Engage kill switch? This halts ALL trading.")) killSwitch.mutate(); }}
              disabled={killSwitch.isPending}
            >
              <Skull size={10} className="mr-1" /> Kill All
            </Button>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <Card className="bg-card border-card-border">
              ...
              <CardContent className="p-2.5 pt-1">
                <div className="text-xl font-bold text-primary">{sys?.scanner?.activeSource ? "ACTIVE" : "IDLE"}</div>
              ...
              <CardContent className="p-2.5 pt-1 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] font-bold">{isRunning ? "LIVE" : "OFF"}</span>
                  </div>
                ...
                <div className="space-y-1 max-h-56 overflow-y-auto">
                  {tokenList.length === 0 ? (
                    ...
                    <div key={t.id} className="flex items-center justify-between border-b border-border/20 pb-1">
                      <div>
                        <p className="text-[9px] font-bold uppercase">{t.tokenSymbol}</p>
                ...
                {historyList.map((h: any) => (
                  <div key={`${h.kind}-${h.id}`} className="flex items-center justify-between text-[9px]">
                    <div className="flex items-center gap-1.5">
                      ...
                      {h.kind === "rejected" && <span className="text-[8px] text-losses">RUG_FILTERED</span>}
                    </div>
                    {h.pnlUsd !== null && (
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-0.5">
                {(readiness.keys as any[]).map((k: any) => (
                  <div key={k.key} className="flex items-center gap-1 text-[8px]">
                    {k.present ? <CheckCircle2 size={8} className="text-gains shrink-0" /> : <XCircle size={8} className="text-losses shrink-0" />}
                    <span className={k.present ? "text-muted-foreground" : "text-losses/70"}>{k.label}</span>
                  </div>
                ))}
              </div>
            </CardContent>
