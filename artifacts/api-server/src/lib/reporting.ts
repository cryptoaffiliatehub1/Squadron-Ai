...
import { generateDailyReport, readDailyReport, isPaperMode, saveFailedReport } from "./paperTrading";
...
const PAPER_BANNER = isPaperMode() ? "\n⚠️  SIMULATION — NO REAL MONEY TRADED\n" : "";
...
async function sendWhatsApp(message: string): Promise<void> {
  ...
  const fullMessage = (isPaperMode() ? "⚠️ SIMULATION MODE\n" : "") + message;
  for (const to of [WHATSAPP_1, WHATSAPP_2]) {
    await client.messages.create({ from: `whatsapp:${from}`, to: `whatsapp:${to}`, body: fullMessage });
    logger.info({ to }, "WhatsApp report sent");
  }
}
